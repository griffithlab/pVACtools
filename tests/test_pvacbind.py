    ...
    def test_pvacbind_pipeline(self):
        with patch('requests.post', unittest.mock.Mock(side_effect = lambda url, data, files=None: make_response(
            ...
            for file_name in (
                ...
                if file_name == 'sample.name.filtered.tsv':
                    import shutil
                    shutil.copy(output_file, os.path.join(self.pVac_directory, '..', '..', '.temp_backup', 'pvacbind', 'testncoutfile', 'gend.Test.filtered.tsv'))
