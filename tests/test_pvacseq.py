...
def generate_class_ii_call(method, allele, path, input_path, sample_name):
    reader = open(os.path.join(
        input_path,
        "MHC_Class_II",
        "tmp",
        sample_name+"_31.fa.split_1-48"
    ), mode='r')
    text = reader.read()
    reader.close()
...
class PvacseqTests(unittest.TestCase):
    ...
    def test_pvacseq_pipeline(self):
        with patch('requests.post', unittest.mock.Mock(side_effect = lambda url, data, files=None: make_response(
            ...
            mock_request.assert_has_calls([
                generate_class_ii_call('nn_align', 'DRB1*11:01', self.test_data_directory, output_dir.name, 'sample.name')
            ])
