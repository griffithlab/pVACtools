...
class JunctionToFastaTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.fasta_file = gunzip_file(os.path.join(cls.test_data_dir, 'inputs', 'all_sequences_chr1.fa.gz'))
        cls.combined_df = pd.read_csv(os.path.join(cls.test_data_dir, 'results', 'Test.combined.tsv'), sep='\t', na_values="NA", dtype={'transcript_support_level': str})
        cls.gtf_df = pd.read_csv(os.path.join(cls.test_data_dir, 'results', 'Test.filtered.gtf.tsv'), sep='\t', na_values="NA", dtype={'transcript_support_level': str})
    ...
    def test_downstream_sequence_length(self):
        ...
        expected_file = os.path.join(self.test_data_dir, 'results', 'Test.transcripts.downstream_sequence_length.fa')
