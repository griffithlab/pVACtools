...
class CalculateReferenceProteomeSimilarityTests(unittest.TestCase):
    ...
    def test_calculate_self_similarity_with_peptide_fasta_pvacsplice(self):
        input_file = os.path.join(self.test_data_dir, 'input.pvacsplice.tsv')
        input_fasta = os.path.join(self.test_data_dir, 'input.pvacsplice.fasta')
        output_file = tempfile.NamedTemporaryFile()
        metric_file = "{}.reference_matches".format(output_file.name)
        ...
        self.assertTrue(cmp(
            output_file.name,
            os.path.join(self.test_data_dir, "output.peptide_fasta.pvacsplice.tsv"),
        ))
        self.assertTrue(cmp(
            metric_file,
            os.path.join(self.test_data_dir, "output.peptide_fasta.pvacsplice.tsv.reference_matches"),
        ))
        os.remove(metric_file)
    ...
    def test_calculate_self_similarity_with_aggregated_tsv_and_peptide_fasta_pvasplice(self):
        ...
        input_fasta = os.path.join(self.test_data_dir, 'Test.pvacsplice.fasta')
        output_file = tempfile.NamedTemporaryFile(suffix='.tsv')
        metric_file = "{}.reference_matches".format(output_file.name)
        output_aggregated_metrics_file = output_file.name.replace(".tsv", ".metrics.json")
        self.assertFalse(CalculateReferenceProteomeSimilarity(
        ...
        self.assertTrue(cmp(
            output_file.name,
            os.path.join(self.test_data_dir, "output.pvacsplice.aggregated.peptide_fasta.tsv"),
        ))
        self.assertTrue(cmp(
            metric_file,
            os.path.join(self.test_data_dir, "output.pvacsplice.aggregated.peptide_fasta.tsv.reference_matches"),
        ))
        os.remove(metric_file)
    ...
    def test_calculate_self_similarity_with_aggregated_tsv_and_peptide_fasta_mouse(self):
        ...
        self.assertTrue(cmp(
            metric_file,
            os.path.join(self.test_data_dir, "output.aggregated.peptide_fasta.mouse.tsv.reference_matches"),
        ))
        os.remove(metric_file)
