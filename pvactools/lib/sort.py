...
def default_sort_from_pd_dict(rows, top_score_metric):
    if top_score_metric == 'median':
        ...
        sorted_rows = sorted(sorted_rows, key=lambda row: (row['Corresponding Fold Change'] if row['Corresponding Fold Change'].isinstance(float) else float('inf')), reverse=True)
