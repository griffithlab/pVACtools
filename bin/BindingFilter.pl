...
if ( !$opt_f || !$opt_i || !$opt_o ) {
    ...
    print
"-f <FOF containing list of parsed epitope files for different allele-length combinations (same sample)>\n";
    print
"-o <Output .xls file containing list of filtered epitopes based on binding affinity for each allele-length combination per gene>\n";
    print
"-c <Minimum fold change between mutant binding score and wild-type score. The default is 0, which filters no results, but 1 is often a sensible default 
   (requiring that binding is better to the MT than WT)>\n";
    print
"-b <report only epitopes where the mutant allele has ic50 binding scores below this value ; default 500>\n";
...
if ( defined( $variants{"header"} ) ) {
    ...
    print OUT join(
        "\t",
        (
            $variants{"header"},  "GeneName",
            "HLAallele",          "PeptideLength",
...
while ( my $file = <FOF> ) {
  ...
  FILELINE:
    while (<FILE>) {
        ...
        my ( $gene_name, $point_mutation, $sub_peptide_position, $mt_score,
            $wt_score, $mt_epitope_seq, $wt_epitope_seq, $fold_change, )
          = split( "\t", $line );
    ...
    foreach my $sample ( sort keys %{ $prediction{$mode} } ) {
      LENGTH:
        foreach my $length ( sort keys %{ $prediction{$mode}->{$sample} } ) {
          GENES:
            foreach my $gene (
                sort @{ $prediction{$mode}->{$sample}->{$length}->{genes} } )
            {
                # BEST
    ...
    foreach my $gene ( sort keys %{ $best{$sample} } ) {
        foreach my $entry ( @{ $best{$sample}->{$gene}->{GENES} } ) {
            if (   ( $$entry->{mt_score} < $threshold )
                && ( $$entry->{fold_change} > $minimum_fold_change ) )
            {
                #write out epiptopes
                #                print OUT $entry;
                ...
                if ( defined($variant_file) ) {
                    ...
                    if ( defined( $variants{$key} ) ) {
                        ...
                        print "Couldn't find variant for "
                          . $gene . " "
