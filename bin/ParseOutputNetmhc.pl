...
for my $protein_name ( sort keys %{ $netmhc_results->{$protein_type} } ) {
    for my $variant_aa (
        sort keys %{ $netmhc_results->{$protein_type}->{$protein_name} } )
    {
        %position_score =
          %{ $netmhc_results->{$protein_type}{$protein_name}{$variant_aa} };
        my @positions =
          sort { $position_score{$a} <=> $position_score{$b} }
          keys %position_score;
        my $total_positions = scalar @positions;

        for ( my $i = 0 ; $i < $total_positions ; $i++ ) {

            if ( $epitope_seq->{'MT'}->{$protein_name}->{$variant_aa}
                ->{ $positions[$i] } ne
                $epitope_seq->{'WT'}->{$protein_name}->{$variant_aa}
                ->{ $positions[$i] } )
              ...
              # Filtering if mutant amino acid present
            {
                ...
                print OUT join( "\t",
                    $protein_name, $variant_aa, $position,
                    $position_score{$position} )
                  . "\t";
                print OUT $netmhc_results->{'WT'}->{$protein_name}
                  ->{$variant_aa}->{$position} . "\t";
                print OUT $epitope_seq->{'MT'}->{$protein_name}->{$variant_aa}
                  ->{$position} . "\t";
                print OUT $epitope_seq->{'WT'}->{$protein_name}->{$variant_aa}
                  ->{$position} . "\t";
                my $fold_change =
                  $netmhc_results->{'WT'}->{$protein_name}->{$variant_aa}
                  ->{$position} / $position_score{$position};
...
sub make_hashes_from_input {
    ...
    while ( my $line = <IFH> ) {
        ...
        if ( $line =~ /^Entry/ ) {
            ...
            $netmhc_results{$protein_type}{$protein_name}{$variant_aa}
              {$position} = $score;
            $epitope_seq{$protein_type}{$protein_name}{$variant_aa}{$position}
              = $epitope;
        }
    }
