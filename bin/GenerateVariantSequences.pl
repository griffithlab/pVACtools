...
while ( my $line = <IFH> ) {
    ...
    if ( $protein_arr[7] =~ /^([A-Z])(\d+)([A-Z])/ ) {
        my ( $wildtype_amino_acid, $position, $mutant_amino_acid ) =
          ( $1, $2 - 1, $3 );
        ...
        if ( $wildtype_amino_acid ne $arr_wildtype_sequence[$position] ) {
            ...
            my ( $mutation_position, @wildtype_arr ) =
              get_wildtype_subsequence_for_printing(
                $position,     \@arr_wildtype_sequence,
                \@protein_arr, $peptide_sequence_length
              );
            my @mutant_arr = @wildtype_arr;

            $mutant_arr[$mutation_position] = $mutant_amino_acid;
            print_to_output( \@wildtype_arr, \@mutant_arr, \@protein_arr,
                $position );
        }
    }
}
...
sub get_wildtype_subsequence_for_printing {
    my (
        $position,    $arr_wildtype_sequence_ref,
        $protein_arr, $peptide_sequence_length
    ) = @_;
    ...
    if ( distance_from_start( $position, @arr_wildtype_sequence ) <
        $one_flanking_sequence_length )
    {
        @wildtype_arr =
          @arr_wildtype_sequence[ 0 ... ( $peptide_sequence_length - 1 ) ];
        $mutation_position = $position;
    }
    elsif ( distance_from_end( $position, @arr_wildtype_sequence ) <
        $one_flanking_sequence_length )
    {
        @wildtype_arr =
          @arr_wildtype_sequence[ (
              $#arr_wildtype_sequence - $peptide_sequence_length + 1 )
          ... $#arr_wildtype_sequence ];
        $mutation_position = $peptide_sequence_length -
          distance_from_end( $position, @arr_wildtype_sequence ) - 1;
    }
    elsif (
        (
            distance_from_start( $position, @arr_wildtype_sequence ) >=
            $one_flanking_sequence_length
        )
        && ( distance_from_end( $position, @arr_wildtype_sequence ) >=
            $one_flanking_sequence_length )
      )
    {
        @wildtype_arr =
          @arr_wildtype_sequence[ ( $position - $one_flanking_sequence_length )
          ... ( $position + $one_flanking_sequence_length ) ];
        $mutation_position = ( $peptide_sequence_length - 1 ) / 2;
    }
    else {
      die: Something went wrong during the retrieval of the wildtype sequence
          at $position $!;
...
sub print_to_output {
    ...
    if ($wildtype_arr_ref) {
        for my $designation ( 'WT', 'MT' ) {
            my $arr =
              $designation eq 'WT' ? $wildtype_arr_ref : $mutant_arr_ref;
...
sub print_fasta_entry {
    my %params = @_;
    open( OUTAPP, ">>$output_file" )
      || die "Can't open $output_file for writing.\n";

    my ( $fasta_defline, $fasta_sequence );
    my $identifier =
        $params{'designation'} . "."
      . $params{protein_arr}->[5] . "."
      . $params{protein_arr}->[7];
