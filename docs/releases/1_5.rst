...
- The ``pvacbind run`` command would previously error out if one of the input
  ...
  protein sequence.
- A bug in the ``pvacfuse top_score_filter`` code would previously result
...
- The ``pvacbind run`` command would previously allow fasta input files with
  ...
  pVACbind will now error out if a duplicate fasta header is encountered.
...
- The standalone ``pvacbind top_score_filter`` command would throw an error
...
- Vaxrank was pinned to an older version because newer versions made
  ...
  package has an indirect dependency on an old version of pysam. We
