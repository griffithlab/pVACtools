...
- Users may provide an optional STAR-fusion output file to their pVACfuse run
  ...
  report. Please see the :ref:`output file section <pvacfuse_output_files>` of the documentation for
  more details.
...
- The ``--aggregate-report-evaluation`` parameter in the standalone ``pvacseq
  ...
  conflicts with required positional parameters. The parameter definition was
...
- When running with the --noncanonical flag, the exons.csv file will contain
  exon positions for all possible transcript combinations. However, the transcripts
