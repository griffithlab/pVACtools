...
- ``VAF Clonal`` : Tumor DNA variant allele frequency (VAF) to determine whether the variant is clonal. A rough estimate of this value is determined automatically from the VAFs in the input data (see pVACtools docs for further details) but can be adjusted by the user (see below).
...
- ``Allele Expression for Passing Variants`` : allele expression cutoff for passing variants. (default: 2.5 Expression * VAF)

**Predicted Binding Affinity**

Binding affinity is measured by IC50 (peptide concentration required for 50% of displacement of a reference peptide to an MHC groove). Lower IC50 values signify better binding affinity. A common threshold for considering a peptide to be a strong binder is 500 nM. 

- ``Binding Threshold``: IC50 value cutoff for a passing neoantigen. (default: 500 nM)

We also list the `Binding threshold for inclusion in the Metric File`. This parameter determines how many peptides the user wants to include in the peptide detailed view. As pVACview cannot reasonably query the many thousands or millions of possible peptides tested with pVACseq, only peptides with a binding of 5000 nM or higher are exported, which captures information about different peptide candidates from the same mutation but also excludes many options that have poor binding.
...
(`TSL <https://useast.ensembl.org/info/genome/genebuild/transcript_quality_tags.html>`_) provides information on degree to which transcript isoforms are supported by experimental evidence. The existing TSL levels are: TSL1, TSL2, TSL3, TSKL4, TSL5, TSLNA, with TSL1 being the highest confidence.  We suggest using a high TSL cutoff. Default is set to be TSL1. 

- ``Maximum TSL`` : cutoff TSL level for a passing candidate. (default: 1)

Percentile rank is a method used to predict binding affinity of a peptide by comparing it to a set of peptides with similar sizes. A lower percentile rank indicates stronger affinity. Percentile rank of less than 2% are generally
