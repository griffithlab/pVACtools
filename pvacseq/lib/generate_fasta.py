...
def main(args_input = sys.argv[1:]):
    ...
    parser.add_argument("-d", "--downstream-sequence-length", help="Cap to limit the downstream sequence length for frameshifts when creating the fasta file.")
    ...
    for line in tsvin:
        ...
        if variant_type == 'FS':
            ...
            if args.downstream_sequence_length and len(downstream_sequence) > int(args.downstream_sequence_length):
                downstream_sequence = downstream_sequence[0:int(args.downstream_sequence_length)]
